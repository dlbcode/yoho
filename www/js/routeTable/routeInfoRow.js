...
function routeInfoRow(rowElement, fullFlightData, routeIds, routeIndex) {
    ...
    detailCell.innerHTML = `
        <div class='route-details' style='display: flex; flex-wrap: nowrap; align-items: flex-start;'>
            ${flight.route.map((segment, idx) => `
                <div class='segment-details' style='flex: 0 1 auto; margin-right: 20px;'>
                    <div class='segment-header'>${segment.flyFrom} > ${segment.flyTo}</div>
                    <div class='segment-info'>Departure: ${new Date(segment.local_departure).toLocaleTimeString()}</div>
                    <div class='segment-info'>Arrival: ${new Date(segment.local_arrival).toLocaleTimeString()}</div>
                    <div class='segment-info'>Duration: ${((new Date(segment.local_arrival) - new Date(segment.local_departure)) / 3600000).toFixed(1)} hrs</div>
                    ${idx < flight.route.length - 1 ? `<div class='segment-info'>Layover: ${formatLayover(flight, idx)}</div>` : ''}
                </div>
            `).join('')}
        </div>
